...
class VQA:
    ...
    def loadRes(self, resFile, quesFile, smallSet=False):
        """
        ...
        if smallSet :
            tmp_questions = []
            tmp_annotations = []
            for qid in annsQuesIds :
                tmp_questions.append(self.qqa[int(qid)])
                tmp_annotations.append(self.qa[int(qid)])
            self.questions['questions'] = tmp_questions
            self.dataset['annotations'] = tmp_annotations
            self.createIndex()
            print('==>Testing on small set ({}) of dataset'.format(len(self.questions['questions'])))
            #for i in range(10):
            #    print(annsQuesIds[i],
            #          self.dataset['annotations'][i]['question_id'])
